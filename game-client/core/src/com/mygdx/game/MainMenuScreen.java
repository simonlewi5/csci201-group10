...
public class MainMenuScreen implements Screen {
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isTouched()) {
            ...
            if (touchX >= bstartX && touchX <= bstartX + buttonWidth &&
                    touchY >= bstartY + (buttonHeight + buttonSpacing) * 2 && touchY <= bstartY + (buttonHeight + buttonSpacing) * 2 + buttonHeight) {
                // Start Game button clicked
                game.setScreen(new GameScreen(game));
                dispose();
            } else if (touchX >= bstartX && touchX <= bstartX + buttonWidth &&
                    touchY >= bstartY + buttonHeight + buttonSpacing && touchY <= bstartY + (buttonHeight + buttonSpacing) * 2) {
                // Join Game button clicked
                // Implement Join Game functionality
            } else if (touchX >= bstartX && touchX <= bstartX + buttonWidth &&
                    touchY >= bstartY && touchY <= bstartY + buttonHeight) {
                // Exit button clicked
                Gdx.app.exit();
            }
